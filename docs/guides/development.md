...
Документ описывает базовые процессы разработки проекта «Союзлифт Аудит». Информация дополняет архитектурное описание из [docs/architecture/v3.md](../architecture/v3.md) и план задач в [AGENTS.md](../../AGENTS.md).
...
- Файлы базы данных и вложения размещаются в каталоге `backend/` (подкаталоги `db/` и `media/`). По умолчанию используется файловое хранилище `FileSystemStorage`.
- На всех окружениях медиа раздаются веб-сервером из `MEDIA_ROOT`; Django контролирует только загрузку и удаление файлов.
- Резервное копирование SQLite и каталога медиа выполняется скриптами из `scripts/`.

## Документация

- Любые архитектурные или процессные изменения отражайте в `docs/architecture/v3.md` и `AGENTS.md`.
- Для пользовательских инструкций (аудиторы, администраторы) поддерживайте раздел `docs/` в актуальном состоянии.
- Дополнительные руководства оформляйте в отдельных файлах внутри `docs/` с понятными именами.
